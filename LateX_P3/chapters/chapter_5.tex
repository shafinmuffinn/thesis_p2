...
\begin{figure}[htbp]
\centering
\IfFileExists{images/fig_5_1_workflow.png}{\includegraphics[width=\textwidth]{images/fig_5_1_workflow.png}}{\fbox{\parbox{0.86\textwidth}{\centering Missing image placeholder: \texttt{images/fig\_5\_1\_workflow.png}. Place the three-component workflow diagram here.}}}
...
\section{Final-Phase Analysis Scripts}
\label{sec:p3_scripts}

The analyses of Chapters~\ref{chap:crosssubject}--\ref{chap:audit} are produced by the scripts in Table~\ref{tab:p3_scripts}. Apart from \texttt{cv\_pipeline.py}, each runs on cached features, logits or trained heads and needs no encoder training; \texttt{p3\_stats.py} generates every corrected test, confidence interval and figure table cited in those chapters from the result CSVs.

\begin{table}[htbp]
\centering
\small
\caption{Final-phase scripts, their outputs, and where the results are reported.}
\label{tab:p3_scripts}
\begin{tabular}{p{3.9cm} p{5.0cm} p{4.6cm}}
\hline
\textbf{Script} & \textbf{Output} & \textbf{Reported in} \\
\hline
\texttt{cv\_pipeline.py} & \texttt{cv5\_fusion.csv}, \texttt{cv5\_fusion\_subjectnorm.csv} & Chapters~\ref{chap:crosssubject}, \ref{chap:calibration} \\
...
\texttt{p3\_stats.py} & \texttt{p3\_stats/*.csv} & All corrected tests and CIs \\
\hline
\end{tabular}
\end{table}
...
Minor numerical differences between runs are expected due to non-deterministic CUDA operations. Results reported in Chapters~\ref{chap:within}--\ref{chap:audit} are from a single deterministic forward pass on the saved logits, so logit-derived metrics such as accuracy and suppression matrix counts are fully deterministic given the saved artefacts.