...
First, the \textbf{fusion problem}. Learned fusion modules are routinely reported to outperform simple late fusion, but those comparisons are only as good as the protocol behind them. On a corpus with 280 training trials per participant, a 2.28-million-parameter attention module may have too little data to learn more than averaging provides, and a fair comparison requires that neither side benefits from selection on the test set.
...
\begin{enumerate}
    \item \textbf{Unsupervised subject calibration for trimodal fusion (Chapter~\ref{chap:calibration}).} Per-participant standardisation of the fusion features, computed from about 20 unlabelled five-second clips, lifts every learned fusion head on unseen participants from 3--4.5 points below naive averaging to 11--14 points above it. The chapter characterises how much calibration data is needed, shows that a neutral-only enrolment session fails, and shows with linear probes that what the calibration removes is linearly decodable participant identity.
    \item \textbf{An evaluation-integrity audit (Chapter~\ref{chap:audit}).} Test-set epoch selection inflated the Pre-Thesis~II fusion results by 5.3--7.5 points; trial alignment across modalities cannot be verified from the released data; and the suppression matrix is statistically indistinguishable from the EEG classifier's own error structure. Each test is cheap, reusable and applicable to other work on EAV.
    \item \textbf{The first subject-independent evaluation of trimodal fusion on EAV (Chapter~\ref{chap:crosssubject})}, which localises the loss on new participants to the vision modality.
    \item \textbf{A trimodal attention fusion module with a zero-EEG inference path (Chapters~\ref{chap:methodology} and~\ref{chap:within})}, trained with softhard modality dropout, which remains accurate when EEG is absent.
