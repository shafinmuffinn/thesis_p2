...
\textbf{Stage C---Fusion Training and Evaluation.} The \texttt{TrimodalAttentionFusion} module is trained on the cached per-subject features for a fixed schedule and evaluated once, at the final epoch; no epoch is chosen by looking at test data (Section~\ref{sec:audit_selection} explains why this differs from the Pre-Thesis~II protocol). Per-subject accuracy is appended to a CSV result file. Separately, the per-modality Stage A logits are used to compute a naive late-fusion baseline with no training required and to populate the coherence suppression matrix.

\begin{figure}[htbp]
\centering
\IfFileExists{images/fig_4_1_trimodal_pipeline.png}{\includegraphics[width=0.9\textwidth]{images/fig_4_1_trimodal_pipeline.png}}{\fbox{\parbox{0.86\textwidth}{\centering Missing image placeholder: \texttt{images/fig\_4\_1\_trimodal\_pipeline.png}. Upload the original pipeline diagram here.}}}
...
\begin{figure}[htbp]
\centering
\IfFileExists{images/fig_4_2_attention_arch.png}{\includegraphics[width=0.9\textwidth]{images/fig_4_2_attention_arch.png}}{\fbox{\parbox{0.86\textwidth}{\centering Missing image placeholder: \texttt{images/fig\_4\_2\_attention\_architecture.png}. Upload the original TrimodalAttentionFusion architecture diagram here.}}}
...
\begin{figure}[htbp]
\centering
\IfFileExists{images/fig_4_3_suppression_criteria.png}{\includegraphics[width=0.9\textwidth]{images/fig_4_3_suppression_criteria.png}}{\fbox{\parbox{0.86\textwidth}{\centering Missing image placeholder: \texttt{images/fig\_4\_3\_suppression\_criteria.png}. Upload the original suppression-event criteria diagram here.}}}
...
Cross-population consistency does not establish that a pattern reflects the participants rather than the classifiers. In a cue-based corpus a matrix cell can be populated entirely by one classifier's systematic errors. Each cell is therefore compared against a class-conditional permutation null: within each participant and each cued class, EEG's (prediction, confidence) pairs are shuffled across trials while audio and vision are held fixed, and the matrix is recomputed 2{,}000 times. The shuffle preserves each classifier's per-class confusion exactly and destroys only the trial-level pairing between the modalities, so a cell that exceeds the null carries coupling between audio-visual and EEG predictions that independent errors cannot produce. Two-sided permutation $p$-values are Holm-corrected over the 20 off-diagonal cells. Results are in Section~\ref{sec:audit_suppression}.
...
The softhard scheme operates on each training sample independently. With probability $p = 0.5$ the sample is left intact; otherwise one of the three modalities is chosen uniformly at random and its feature vector is replaced by zeros. The surviving modalities are then rescaled by $3/n_{\mathrm{active}}$, where $n_{\mathrm{active}}$ is the number of modalities present, so that the expected input energy per sample does not depend on how many modalities were dropped:
\begin{equation}
  \tilde{x}^{(m)} = \frac{3\, k_m}{\sum_{m'} k_{m'}}\, x^{(m)}, \qquad k_m \in \{0, 1\},
\end{equation}
where $k_m = 0$ for the dropped modality and 1 otherwise. Labels are unchanged, and the batch size is unchanged. Over training, the module therefore sees each sample with all three modalities about half the time and with each two-modality subset about one-sixth of the time, forcing each modality token to carry evidence that is useful on its own. Dropout is applied only during training. (The Pre-Thesis~II report described a different scheme, in which every batch was expanded into four sub-batches; that description did not match the implementation that produced its results, and is corrected here; see Section~\ref{sec:p3_challenges}, C20.)

The configuration that matters most for deployment is the audio-visual one: EEG zeroed, audio and vision present, as it would be for any user without an EEG headset. Because EEG is dropped in about one-sixth of training samples, the module is trained on this condition directly while still being trained on complete inputs. After training, accuracy with EEG zeroed at inference is the primary robustness metric (Sections~\ref{sec:modality_dropout_results} and~\ref{sec:cal_ranking}).

\section{Evaluation Protocols}
\label{sec:evaluation_protocols}

Three evaluation protocols are used in this report, each answering a different question.

\textbf{Within-subject} (Chapter~\ref{chap:within}). Each participant's 400 trials are split 280/120 by the EAV repository's stratified split; encoders and fusion heads are trained on a participant's 280 training trials and tested on their 120 test trials; results are averaged over the 42 participants. This answers how well the system recognises emotion in a person it has already studied, and it is the protocol used by all prior trimodal fusion work on EAV.

\textbf{Cross-subject} (Chapter~\ref{chap:crosssubject}). Five-fold subject-wise cross-validation: every participant is held out exactly once, and the encoders and fusion heads for that fold are trained only on the other participants, with epoch selection on three inner-validation participants. This answers how well the system recognises emotion in a person it has never seen.

\textbf{Cross-subject with calibration} (Chapter~\ref{chap:calibration}). As cross-subject, but each participant's fusion features are standardised with statistics from a short unlabelled recording of that participant. This answers how well the system works for a new user who provides a brief enrolment session without labels.

Under every protocol, the reported epoch of every trained model is chosen without reference to the data it is scored on.
