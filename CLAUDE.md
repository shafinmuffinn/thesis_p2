...
### ⚡ RESUME HERE (24 Sep 2026) — submission Sat 26 Sep 23:00 Dhaka
- **⚠ TURNITIN RULE for the P3 report (decided 24 Sep, apply when generating the final report):** the P2 report is already on Turnitin, so P3 must NOT reuse P2 wording. Chapters 1–5 are to be **summarised in fresh wording** (condensed background, not the P2 text), and the Table of Contents restructured to match. Measured verbatim overlap with `LateX/` (sentence-level, 24 Sep): Ch2 89%, Ch5 89%, Ch4 81%, Ch3 55%, Ch1 12%; also P2-era text **outside** Ch1–5: engineering challenges C1–C13 (`chapter_7.tex`, 71% overall), Ch6 §6.1–6.2 per-modality/naive (~460 words), front matter (declaration/acknowledgement/approval are templates). `chapter_7_crosssubject.tex` and C14–C24 were written after the P2 submission (not on Turnitin; confirm). Rewrite must keep every number and citation intact and re-run `python verify_latex.py`. Open questions for the user are listed in P3_PLAN.md.
- **User answers 24 Sep:** similarity limit ~20% INCLUDING refs + template pages, compared against our own P2 -> rewrite body to ~0% verbatim; also rewrite C1-C13 and Ch6 per-modality/naive text; P2 diagrams reusable; no page limit; submission **26 Sep 23:00 Dhaka**; defence talk 5-10 min. Experiments chosen: A calibrated-naive control, B labelled few-shot, D per-modality calibration, E attention-weight analysis (see P3_PLAN.md).
- **Out of scope (24 Sep): MERCL and sequence features + 6-way CMA** — no time to tune their hyperparameters. Future work only.
...
- **Original experiments done** (4 more added 24 Sep: A/B/D/E, see above). Scripts: `calibration_study.py`, `identity_probe.py`, `p2_leakfree.py`, `label_sequence_check.py`, `suppression_audit.py`, `p3_stats.py` (all tables → `results/p3_stats/`), `dynamic_late_fusion.py`.
- **FINAL headline (25 Sep, supersedes the next line):** calibration, not the fusion operator, is what matters without labels — calibrated averaging 74.9% ≈ every calibrated head (all n.s.). With 20–50 LABELLED clips per new user, learned heads beat labelled averaging (+3.2–4.8 pp at 50), dropout-trained attention first and most (81.9%). Vision identity ≈70% of the calibration gain. Attention weights are dynamic; favour audio when right; not EEG.
- **Earlier headline (superseded by the calibrated-averaging control):** per-participant feature standardisation (`--subject-norm`) lifts cross-subject learned fusion 58→74–76% vs naive 62.2%; holds with 20 unlabeled disjoint calibration clips (~100 s); neutral-only calibration fails; skewed needs 50–100. Identity linearly decodable (vision 100%, audio 97%, EEG 82%) → chance after calibration.
...
- **EEGNet OVERFITS at fold scale.** 60 epochs: inner-val loss 1.4469 / acc 0.3833. 200 epochs: 1.6389 / 0.3567, while training loss kept falling. An earlier reading of "still climbing" at epoch 60 was noise on the accuracy; the val *loss* was already near its minimum. Fixed by making `EEG_EPOCHS` (120) an upper bound and keeping the best inner-val epoch, with `EEG_PATIENCE=25` early stopping — `Trainer_uni` only prints validation and never restores a best state, so the old code saved the final (overfit) epoch. EEG still appears to lose the least cross-subject (~6 pp), so the big penalty is on vision and the fusion heads.
